...
import { Box, Card, CardContent, CardMedia, CardActionArea, Typography, Button, Divider } from '@mui/material/';
...
export default function CategoriesMain() {
  ...
  if (!selectedCategory) {
    return <Typography variant="h6" sx={{ textAlign: 'center', marginTop: '20px' }}>Category Not Found</Typography>;
  }
    ...
    <Box sx={{ width: '100%' }}>
      ...
      {selectedCategory.type.map((type, index) => (
        <Box key={index} sx={{ marginBottom: '20px' }}>
          <Typography variant="h6" sx={{ marginX: '11%', textAlign: 'center' }}>{type.name}</Typography>
          <Typography variant="body1" sx={{ marginX: '11%', textAlign: 'center', marginBottom: '10px' }}>{type.description}</Typography>
          <Button
            onClick={() => handleClick(type.name)}
            variant="contained"
            sx={{ fontFamily: 'Poppins, sans-serif', borderRadius: '60px', color:'#F7FFA', backgroundColor:'#FCC783', marginX: '45%', marginY: '2%' }}
          >
            View Variants
          </Button>
